...
export default function FilterCharacters({ link }: { link: string }) {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            ...
            sortBy: searchParams.get('sortBy') ?? "random",
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if(values.show_all){
            params.set('show_all', values.show_all.toString())
        }
                ...
                {params.get('pagination') === "false" && <FormField
                    ...
                    render={({ field }) => (
                        <FormItem className="w-[95%] mx-auto mt-5">
                            ...
                            <FormControl>
                                <Slider
                                    disabled={form.getValues().show_all}
                                    onValueChange={(value) => field.onChange(value[0].toString())}
                                    defaultValue={[parseInt(field.value)]}
                                    max={714}
                                    min={CHARACTERS_PER_PAGE}
                                    step={CHARACTERS_PER_PAGE}
                                    className={`${form.getValues().show_all && "opacity-20"}`}
                                />
                            </FormControl>
                            <FormDescription>
                                {form.getValues().show_all ? "All" : field.value}
                            </FormDescription>
                            <FormMessage />
                        </FormItem>
                    )}
